...
/*
---------------------------------------
コード生成に必要な、メモリ格納場所を決める
---------------------------------------
*/
typedef struct memory_set * memory_set_t;
struct memory_set{
	int cur;
	int max;
};
...
/* syntree_info構造体 */
struct syntree_info{
  int num_var;	//stmt_tで用いる。変数の個数を管理。
  
  int kind;      //kind:0 expression(初期値)    1:int    2:id　　3:eaxのまま計算可能な関数  //stmt_tでも用いる。kind:1 関数の最後にあるreturn
  int rel_val;	//expr_tで用いる。計算結果格納場所を管理。+rel で相対番地が出る。 1:整数値そのもの    2:%d(ebp)の%dの値
};
...
typedef struct fun_def
{
  ...
  int num_var;			/* 関数内の変数の数 */
  int memory;			/* 計算結果格納に使う最大メモリブロック量 */
...
expr_t mk_expr_paren(char * filename, int line, expr_t e, memory_set_t m);
...
expr_t mk_expr_un_op(char * filename, int line, op_kind_t op, expr_t e, memory_set_t m);
