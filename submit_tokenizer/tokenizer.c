...
token next_tok(tokenizer_t t)
{
  ...
  if( isdigit(t->c) ){
    ...
    nextget(t);
      ...
      nextget(t);
      while( (t->c=='_') || (t->c>='A' && t->c<='Z') || (t->c>='a' && t->c<='z') || (isdigit(t->c)) ){
       write_buf(s,t->c);
       nextget(t);
      }
...
void judge_tok(char *s,tokenizer_t t)
{
  if(strcmp("break",s) == 0)
    {
    ...
    {
      t->tok.kind = tok_id;
      clear_buf(t->tok.identifier);
      write_str_buf(t->tok.identifier,s);
    }
}
...
void print_tok(tokenizer_t t)
{
  if(t->tok.kind == tok_break)
    ...
    {
      printf("TOK_ID (%s)",cur_buf(t->tok.identifier) );
    }
